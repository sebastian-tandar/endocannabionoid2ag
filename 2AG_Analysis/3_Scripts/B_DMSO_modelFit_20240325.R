...
outputwd <- "2_CreatedData/B_ControlFits_20240325/"
...
signalModel_nlmixr <- function(){
  ini({
    ...
    t.f_signal = psoPrefit_parameters['t.f_signal'] %>% log()
  ...
  model({
    ...
    # state initiation
    m_extracellular(0) = m_dose_N2A * f_signal * N_N2A_eff
    m_intracellular(0) = 0
    
    # derived parameters
    k_fill <- m_dose_N2A * (1 - f_signal) / (m_pool_ss_N2A * 30 * 60)
    
    # ODEs
    d/dt(m_extracellular) = k_fill * N_N2A_eff * m_pool_ss_N2A - (k_absorption * N_HEK_eff + k_removal) * m_extracellular
...
signalModel_rxode <- rxode2({
  ...
  # derived parameters
  k_fill <- (m_dose_N2A - m_dose_N2A * f_signal) / (m_pool_ss_N2A * 30 * 60)
...
mainData <- appendFixedPars(mainData, fixedPars, 
                            par_names = c("N_HEK", "N_N2A", 'm_dose_N2A',
                                          "m_pool_ss_N2A", 
                                          "Vd", "Smax", "Ki"))

# parameter search boundaries
initPars <- c(t.k_removal = 5*10^-3, t.k_absorption = 1*10^-7, t.k_degradation = 1*10^-3, t.f_signal = 0.5) %>% log()
lowerBound <- c(t.k_removal = 10^-20, t.k_absorption = 10^-20, t.k_degradation = 10^-20, t.f_signal = 0.01) %>% log()
upperBound <- c(t.k_removal = 1, t.k_absorption = 1, t.k_degradation = 1, t.f_signal = 1) %>% log()
...
# B | Main Fit
fitRes <- nlmixr2(signalModel_nlmixr, mainData, est='focei', 
                  control=list(outerOpt='bobyqa'))
